...
class YoutubeIE(_YoutubeIE):
    ...
    def _download_webpage_handle(
        ...
        if self.enable_custom and self.get_param("use_custom_jsc"):
            self.custom_jsc()
        elif self.enable_custom and self.get_param("extractor_args"):
                extractor_args = self.get_param("extractor_args")
                if "youtube-ejs" in extractor_args:
                    youtube_args = extractor_args["youtube-ejs"]
                    if "repo" in youtube_args and "tfkxsx" in youtube_args["repo"][0]:
                        self.write_debug(f"[CustomYoutubeIE] handling --> youtube-ejs : {youtube_args['repo']}")
                        self.custom_ejs()
    ...
    def custom_ejs(self):
        """TODO EJS 使用自定义github仓库的脚本
        这里是对内置_builtin_source进行覆盖, 直接返回None， 让程序走向_web_release_source， 从而触发我们在_web_release_source中对请求的hook
        """
        from yt_dlp.extractor.youtube.jsc._builtin.ejs import EJSBaseJCP as jsp
        def _builtin_source(self, script_type):
            if script_type.value == "core":
                return None
        jsp._builtin_source = _builtin_source
