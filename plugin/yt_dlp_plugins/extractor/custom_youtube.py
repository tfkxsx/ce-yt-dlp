...
class YoutubeIE(_YoutubeIE):
    ...
    def _download_webpage_handle(
        ...
        if self.enable_custom and self.get_param("extractor_args"):
            extractor_args = self.get_param("extractor_args")
            if "youtube-ejs" in extractor_args:
                youtube_args = extractor_args["youtube-ejs"]
                if "repo" in youtube_args:
                    self.write_debug(
                        f"[CustomYoutubeIE] handling --> use custom youtube-ejs : {youtube_args['repo']}"
                    )
                    # 使用自定义 github 仓库的脚本
                    # 这里是对内置 _builtin_source 进行覆盖, 直接返回None， 
                    # 让程序走向 _web_release_source， 
                    # 从而触发我们在 _web_release_source 中对请求的hook
